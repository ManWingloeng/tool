...
extern "C"{
    ViBePlus* Cvibep_new(int num_sam = DEFAULT_NUM_SAMPLES,
         int min_match = DEFAULT_MIN_MATCHES,
         int r = DEFAULT_RADIUS,
         int rand_sam = DEFAULT_RANDOM_SAMPLE)
    {
        // Note: Inside the function body, I can use C++. 
        return new ViBePlus(num_sam, min_match, r, rand_sam);
    }

    void CDeletevibep (ViBePlus* ptr)
    {
         delete ptr;
    }

//     void FrameCapture(ViBePlus* ptr, cv::Mat img){
//         return ptr->FrameCapture(img);
//     }

    // 背景模型初始化
    // Init Background Model.
    void Cinit(ViBePlus* ptr){
        return ptr->init();
    }

    // 处理第一帧图像
    // Process First Frame of Video Query
    void CProcessFirstFrame(ViBePlus* ptr){
        return ptr->ProcessFirstFrame();
    }

    // 运行 ViBe+ 算法，提取前景区域并更新背景模型样本库
    // Run the ViBe+ Algorithm: Extract Foreground Areas & Update Background Model Sample Library.
    void CRun(ViBePlus* ptr){
        return ptr->Run();
    }

    // 提取分割模板
    // Extract the Segment Model
    void CExtractBG(ViBePlus* vibep){
        return vibep->ExtractBG();
    }

    // 根据已经得到的分割模板，计算更新模板；
    // Calculate Update Model from Segment Model
    void CCalcuUpdateModel(ViBePlus* vibep){
        return vibep->CalcuUpdateModel();
    }

    // 更新背景模板
    // Update the Update Model
    void CUpdate(ViBePlus* vibep){
        return vibep->Update();
    }

    // 更新当前像素点样本集方差
    // Update Variance of Sample Set of Current Pixel whose location is (i, j)
    void CUpdatePixSampleSumSquare(ViBePlus* vibep, int i, int j, int k, int val){
        return vibep->UpdatePixSampleSumSquare(i, j, k, val);
    }

    // 更新当前像素点样本集平均值
    // Update Average Value of Sample Set of Current Pixel whose location is (i, j)
    void CUpdatePixSampleAve(ViBePlus* vibep, int i, int j, int k, int val){
        return vibep->UpdatePixSampleAve(i, j, k, val);
    }

    // 获取前景模型二值图像
    // get Foreground Model Binary Image.
    Mat CgetSegModel(ViBePlus* vibep){
        return vibep->getSegModel();
    }

    // 获取更新模型二值图像
    // get Update Model Binary Image.
    Mat CgetUpdateModel(ViBePlus* vibep){
        return vibep->getUpdateModel();
    }
    

//     int* Foo_foobar(Foo* foo, int rows, int cols, unsigned char* imgdata) {
//         cv::Mat img(rows, cols, CV_8UC3, (void *)imgdata);
//         return foo->foobar(img);
//     }

    void CFrameCapture(ViBePlus* ptr, int rows, int cols, unsigned char* imgdata){
//     void CFrameCapture(ViBePlus* ptr, int rows, int cols){
        
        Mat imgg(rows, cols, CV_8UC1, (void *)imgdata);

//         printf("CFrameCapture is ok");
//         namedWindow("Display window");// Create a window for display.
//         imshow("Display window", imgg);
//         waitKey(0);
//         Mat img = imread("/home/todd/Pictures/tichu/筛选后的图片/豹/1.jpg");
        return ptr->FrameCapture(imgg);
    }
}
